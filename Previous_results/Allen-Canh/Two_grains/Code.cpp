...
double summation(double arr[n],int i){
    double sum =0;
    for(int j=0;j<n;j++){
        if(j!=i) sum+= (arr[j]*arr[j] ); 
    }
    return sum;
}
...
int main(){
    ...
    for(int i=0;i<ny;i++){
        for(int  j=0;j<nx;j++){
            if(j<(nx/2)) {
            initial_orientation[i][j][0]=1;
            initial_orientation[i][j][1]=0;}
            else{
                initial_orientation[i][j][0]=0;
            initial_orientation[i][j][1]=1;
            }
        }
}
